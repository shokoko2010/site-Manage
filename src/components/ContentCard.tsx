...
import { GeneratedContent, WordPressSite, ContentType, ArticleContent, LanguageContextType, PublishingOptions } from '@/types/types';
import { ArticleIcon, ProductIcon, ClockIcon, TrashIcon, EditIcon, PublishIcon, ArrowUpRightIcon } from '@/lib/constants';
...
const ContentCard: React.FC<ContentCardProps> = ({ content, site, onEdit, onRemove, showNotification, onUpdateLibraryItem, allSites, onVersionManage }) => {
    const { t } = useLanguage();
            ...
            <div className="p-4 flex-grow flex flex-col">
             ...
             <div className="absolute inset-0 bg-gray-900/70 rounded-xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 p-4">
                <div className="flex items-center space-x-2">
                    ...
                    {onVersionManage && (
                        <button onClick={onVersionManage} className="bg-gray-700 hover:bg-purple-500 text-white p-3 rounded-full transition-colors" title={t('versionHistory')}>
                            <ClockIcon/>
                        </button>
                    )}
