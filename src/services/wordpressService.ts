...
import { WordPressSite, GeneratedContent, PublishingOptions, SiteContext, ContentType, ArticleContent, ProductContent, SitePost, MediaItem } from '@/types/types';
...
export const fetchAllPosts = async (site: WordPressSite): Promise<SitePost[]> => {
    ...
    const fields = 'id,title,content,status,date,link,performance_stats,_embedded';

    do {
        try {
            const response = await apiFetch(`${site.url}/wp-json/wp/v2/posts?per_page=20&page=${page}&status=${statuses}&_fields=${fields}&_embed=`, { headers });
